...
def get_recommendations(title):
    ...
    ove = df2["overview"].iloc[movie_indices]
    hom = df2["homepage"].iloc[movie_indices]
    vot = df2["vote_average"].iloc[movie_indices]
    return_df = pd.DataFrame(
        columns=["Title", "Year", "Genres", "Overview", "Homepage", "Voter Average",]
    )
    return_df["Title"] = tit
    return_df["Year"] = dat
    return_df["Genres"] = gen
    return_df["Overview"] = ove
    return_df["Homepage"] = hom
    return_df["Voter Average"] = vot
    return return_df
    ...
    if flask.request.method == "POST":
        ...
        if m_name not in all_titles:
            ...
            for i in range(len(result_final)):
                ...
                voterAvg.append(result_final.iloc[i][5])
